# Memory Vault Index

> Agent 启动时先读这个文件，按需加载具体内容

## 快速访问

| 文件 | 用途 |
|------|------|
| [NOW.md](/NOW.md) | 当前状态仪表盘 |
| [SOUL.md](/SOUL.md) | 核心人格定义 |
| [SYSTEM.md](/memory/SYSTEM.md) | 多领域调度系统 |

---

## 📈 投资研究 (memory/investing/)

| 文件 | 说明 |
|------|------|
| [README](/memory/investing/00-README.md) | 投资分析框架总览 |
| [投资原则](/memory/investing/01-投资原则.md) | 核心纪律与原则 |
| [宏观分析](/memory/investing/macro/宏观分析框架.md) | 自上而下分析 |
| [行业分析](/memory/investing/industry/行业分析模板.md) | 行业研究框架 |
| [小金属板块分析](/memory/investing/industry/小金属板块分析.md) | 锂钴镍稀土等分析 ⭐ |
| [个股分析](/memory/investing/stock/个股分析清单.md) | 深度研究工具 |
| [观察清单](/memory/investing/portfolio/观察清单.md) | 股票池管理 |
| [投资日志](/memory/investing/logs/日志模板.md) | 交易记录与复盘 |
| [2026板块配置](/memory/investing/strategy/2026年板块配置策略.md) | 年度策略主线 ⭐ |
| [2026执行计划](/memory/investing/strategy/2026年投资执行计划.md) | 具体建仓计划 ⭐ |

### 示例分析
| 文件 | 说明 |
|------|------|
| [紫金矿业](/memory/investing/stock/examples/紫金矿业-601899-分析.md) | 全球化矿业龙头 ⭐ |
| [华胜天成](/memory/investing/stock/examples/华胜天成-600410-分析.md) | AI概念股分析 |

---

## 领域目录

### 🏠 家庭 (memory/home/)
| 文件 | 说明 |
|------|------|
| [README](/memory/home/README.md) | 家庭事务管理指南 |
| calendar.md | 家庭日程表 |
| shopping.md | 购物清单 |
| events.md | 重要事件 |

### 💼 工作 (memory/work/)
| 文件 | 说明 |
|------|------|
| [README](/memory/work/README.md) | 工作事务管理指南 |
| projects.md | 项目概览 |
| tasks.md | 任务清单 |
| meetings.md | 会议记录 |

### 🧠 反思 (memory/reflection/)
| 文件 | 说明 |
|------|------|
| [README](/memory/reflection/README.md) | 个人思考框架 |
| [journal/](/memory/reflection/journal/) | 每日日志 |
| goals.md | 目标规划 |
| values.md | 价值观记录 |

### 🎓 培训 (memory/training/)
| 文件 | 优先级 | 状态 | 最后验证 | 说明 |
|------|--------|------|----------|------|
| [README](/memory/training/README.md) | 🔴 | ✅ active | 2026-02-26 | 培训设计子代理 |
| [lessons/memory-architecture](/memory/lessons/memory-architecture.md) | 🔴 | ✅ active | 2026-02-26 | 记忆系统架构 |

### 📚 研究 (memory/research/)
| 文件 | 说明 |
|------|------|
| [x-openclaw-20260226](/memory/research/x-openclaw-20260226.md) | X/OpenClaw 应用调研 |

---

## 经验教训 (memory/lessons/)

| 文件 | 优先级 | 状态 | 最后验证 | 说明 |
|------|--------|------|----------|------|
| [memory-architecture](/memory/lessons/memory-architecture.md) | 🔴 | ✅ active | 2026-02-26 | 记忆系统三层架构 |

---

## 决策记录 (memory/decisions/)

| 文件 | 说明 |
|------|------|
| 待创建 | 重要决策归档 |

---

## 配置文档

| 文件 | 说明 |
|------|------|
| [AGENTS.md](/AGENTS.md) | Agent 操作手册 |
| [IDENTITY.md](/IDENTITY.md) | 身份定义 |
| [USER.md](/USER.md) | 用户信息 |
| [TOOLS.md](/TOOLS.md) | 工具配置 |
| [EMAIL_CONFIG.md](/EMAIL_CONFIG.md) | 邮件配置 |
| [BACKUP.md](/BACKUP.md) | 备份策略 |
| [OBSIDIAN.md](/OBSIDIAN.md) | Obsidian 集成 |

---

## 健康度扫描

### 🔴 核心知识 (永不归档)
- [x] memory-architecture.md (2026-02-26)
- [x] training/README.md (2026-02-26)

### 🟡 重要知识
- [ ] 待填充

### ⚪ 参考信息
- [ ] 待填充

---

*INDEX 版本: v1.0*  
*最后更新: 2026-02-26*  
*更新者: Claw Mate 🦾*
